...
{
	uint8_t u8_counter;
	for(u8_counter=0;u8_counter<DIO_USED_PINS_NUM;u8_counter++)
	{
   switch(strDio_pins[u8_counter].u8_PortNum)
   {
   case PORT_A :
			{
				if(strDio_pins[u8_counter].u8_Direction==DIO_PIN_DIR_INPUT)
				{
					DDRA &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
					
				}
				else if (strDio_pins[u8_counter].u8_Direction==DIO_PIN_DIR_OUTPUT)
				{
					DDRA |=(1<<(strDio_pins[u8_counter].u8_PinNum));
				}
				if(strDio_pins[u8_counter].u8_InitState==0)
				{
					PORTA &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
					
				}
				else if (strDio_pins[u8_counter].u8_Direction==1)
				{
					PORTA |=(1<<(strDio_pins[u8_counter].u8_PinNum));
				}
	...
	case PORT_B :
	    {
		     if(strDio_pins[u8_counter].u8_Direction==DIO_PIN_DIR_INPUT)
		     {
	        		DDRB &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
			...
			if(strDio_pins[u8_counter].u8_InitState==0)
			{
				PORTB &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
	...
	case PORT_C :
	{
		if(strDio_pins[u8_counter].u8_Direction==DIO_PIN_DIR_INPUT)
		{
			DDRC &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
		...
		if(strDio_pins[u8_counter].u8_InitState==0)
		{
			PORTC &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
		...
		case PORT_D :
		{
			if(strDio_pins[u8_counter].u8_Direction==DIO_PIN_DIR_INPUT)
			{
				DDRD &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
			...
			if(strDio_pins[u8_counter].u8_InitState==0)
			{
				PORTD &=~(1<< (strDio_pins[u8_counter].u8_PinNum));
