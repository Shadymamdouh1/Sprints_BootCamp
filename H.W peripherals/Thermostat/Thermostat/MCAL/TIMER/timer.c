...
{   
	uint8_t u8_counter;
	for(u8_counter=0;u8_counter<CHANNELS_NUMBER;u8_counter++)
	{
		
	
	if(timer_config[u8_counter].EN_channel==TIMER0)
	{
...
{   
	...
	//clr pre scaler  bit field
	  ...
	  // set pre scaler
	  TCCR0 |=( timer_config[u8_counter].EN_Prescaler << TCCR0_CS00);
